...
export const PriceDiscovery = ({ language, onBack }) => {
  ...
  const [localities, setLocalities] = useState(LOCAL_NEIGHBORHOOD_PRICES);
  const [selectedArea, setSelectedArea] = useState(LOCAL_NEIGHBORHOOD_PRICES[0].area);
  const [liveGps, setLiveGps] = useState({
    lat: 18.5089,
    lng: 73.9259,
    accuracy: 'High Precision GPS (Live)',
  ...
  const handleRefreshLocation = () => {
    ...
    GeoService.getCurrentLocation().then(loc => {
      const livePlace = loc.placeName || 'Hadapsar, Pune';
      const liveGpsString = `${livePlace} (Live Location)`;
      
      setLiveGps({
        lat: loc.lat,
        lng: loc.lng,
        accuracy: 'Live Device GPS (±3m)',
        address: loc.placeName,
        trackingStatus: 'ACTIVE',
      });

      // Dynamically insert Live Detected Location at the very TOP of localities
      const liveEntry = {
        area: `📍 ${livePlace} (Live Nearest Selling Area)`,
        distanceKm: 0.2,
        materialPrices: { PCB: 445, Cable: 545, LCD: 92, Motor: 188, Battery: 148 },
        nearbyDemand: 'HIGH',
        note: 'Live GPS detected current location. Real-time highest MPCB buyer offer match.',
        isLiveDetected: true,
      };

      const updated = [liveEntry, ...LOCAL_NEIGHBORHOOD_PRICES.filter(l => !l.isLiveDetected)];
      setLocalities(updated);
      setSelectedArea(liveEntry.area);
      setIsLocating(false);
    });
  };
  ...
  const activeAreaObj = localities.find(l => l.area === selectedArea) || localities[0];
  ...
  const handleSpeakPrice = () => {
    ...
    const text = language === 'mr'
      ? `${activeAreaObj.area} मध्ये ${selectedMaterial} चा थेट विक्री दर ₹${localRate} प्रति किलो आहे.`
      : language === 'hi'
      ? `${activeAreaObj.area} में ${selectedMaterial} का लाइव बिक्री भाव ₹${localRate} प्रति किग्रा है।`
      : `Live selling rate for ${selectedMaterial} in ${activeAreaObj.area} is ₹${localRate} per kg.`;
  ...
  return (
    <div className="p-4 space-y-4 pb-16 bg-slate-50 min-h-full">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white p-4 rounded-2xl shadow-md space-y-3 border border-slate-700">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={isLocating}
            className="flex items-center space-x-1 text-[11px] font-bold bg-white/10 hover:bg-white/20 text-white px-2.5 py-1 rounded-lg border border-white/20 transition-all active:scale-95"
          >
            <RefreshCw className={`w-3 h-3 ${isLocating ? 'animate-spin' : ''}`} />
            <span>{isLocating ? 'Detecting Live GPS...' : 'Refresh Live GPS'}</span>
          </button>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl border border-white/15 text-xs space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-emerald-200 bg-emerald-900/60 border border-emerald-500/40 px-2 py-0.5 rounded font-extrabold">
              {liveGps.accuracy}
            </span>
          </div>
          <p className="text-slate-100 text-[11px] font-bold">{liveGps.address}</p>
        </div>
      </div>
      ...
      {/* Dynamic Nearby Selling Locality Selector */}
      <div className="space-y-1.5 bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
        <label className="text-xs font-bold text-slate-700 flex items-center space-x-1">
          <Compass className="w-4 h-4 text-emerald-600" />
          <span>Nearest Live Selling Locality Rate Index:</span>
        </label>
        <select
          value={selectedArea}
          onChange={e => setSelectedArea(e.target.value)}
          className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs font-black text-emerald-900 focus:outline-none focus:border-emerald-500 cursor-pointer"
        >
          {localities.map(loc => (
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium flex items-center space-x-1 mt-0.5">
                ...
                <span className="font-bold text-slate-800">{activeAreaObj.area}</span>
              </p>
            </div>
          </div>
          ...
          <button
            ...
            title="Read out live price audio"
          >
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">
              Live Selling Rate ({selectedMaterial})
            </span>
      ...
      <div className="space-y-2">
        <h3 className="text-xs font-bold text-slate-700">Nearby Selling Localities Rate Comparison ({selectedMaterial})</h3>
