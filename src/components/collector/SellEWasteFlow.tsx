...
import type { Language, MaterialCategory, RecyclerMatchResult, Lot } from '../../types';
import { INITIAL_MATERIALS } from '../../services/mockData';
import { AIEngine, ClassificationResult, ValuationResult } from '../../services/aiEngine';
import { RecyclerMatcher } from '../../services/recyclerMatcher';
import { LocalDatabase } from '../../services/db';
import { syncEngine } from '../../services/syncEngine';
import { TTSService } from '../../services/ttsService';

interface SellEWasteFlowProps {
  language: Language;
  onBack: () => void;
...
export const SellEWasteFlow: React.FC<SellEWasteFlowProps> = ({
  language,
  onBack,
  onLotCreated,
  isOnline,
  ...
  const [photoUrl, setPhotoUrl] = useState<string>(INITIAL_MATERIALS[0].imageUrl);
  ...
  const [condition, setCondition] = useState<'Mixed' | 'Clean' | 'High Grade' | 'Damaged'>('Mixed');
  const [valuation, setValuation] = useState<ValuationResult | null>(null);
  const [recyclerMatches, setRecyclerMatches] = useState<RecyclerMatchResult[]>([]);
  const [selectedRecycler, setSelectedRecycler] = useState<RecyclerMatchResult | null>(null);
  const [createdLot, setCreatedLot] = useState<Lot | null>(null);

  const handlePhotoSelected = (sampleImg: string, categorySeed: string) => {
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-2xl text-center space-y-4 shadow-md">
            ...
            <div className="pt-3 border-t border-slate-100 text-left space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['Clean', 'Mixed', 'High Grade', 'Damaged'] as const).map(c => (
                  <button
                    key={c}
