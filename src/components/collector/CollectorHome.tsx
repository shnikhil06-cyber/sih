...
import type { Language, CollectorProfile } from '../../types';
import { TTSService } from '../../services/ttsService';

interface CollectorHomeProps {
  profile: CollectorProfile;
  language: Language;
  onNavigate: (screen: 'sell' | 'price' | 'recyclers' | 'lots' | 'earnings' | 'safety') => void;
}

export const CollectorHome: React.FC<CollectorHomeProps> = ({ profile, language, onNavigate }) => {
  const getSpokenText = (key: string) => {
    switch (key) {
      case 'sell':
        return language === 'mr'
  ...
  const handleSpeak = (e: React.MouseEvent, key: string) => {
    e.stopPropagation();
