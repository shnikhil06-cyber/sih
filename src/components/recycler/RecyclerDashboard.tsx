...
import type { Lot, PaymentMethod } from '../../types';
import { LocalDatabase } from '../../services/db';
import { syncEngine } from '../../services/syncEngine';
import { AIEngine, AnomalyAnalysisResult } from '../../services/aiEngine';
import { ReceiptModal } from '../common/ReceiptModal';

export const RecyclerDashboard: React.FC = () => {
  const [lots, setLots] = useState<Lot[]>(() => LocalDatabase.getLots());
  const [selectedLot, setSelectedLot] = useState<Lot | null>(null);

  const [verifiedWeight, setVerifiedWeight] = useState<number>(35);
  const [finalPriceInput, setFinalPriceInput] = useState<number>(15050);
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>('CASH');
  const [anomalyResult, setAnomalyResult] = useState<AnomalyAnalysisResult | null>(null);
  const [receiptLot, setReceiptLot] = useState<Lot | null>(null);
  ...
  const handlePriceChange = (newPrice: number, category: any, weight: number) => {
